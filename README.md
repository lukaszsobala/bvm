...
- **USB passthrough:** Now available. [Adding this feature](https://github.com/Botspot/bvm/commit/9389caa5bcc7548363b0fad999f0e32591f16487) took a bit longer than I expected, but it's here now and works well.
    Note to existing users: to specify a USB device to forward to the VM, go read the _latest_ version of the config file in the github repository to see how to set the new `usb_passthrough` variable. The usb-forwarding instructions in your existing config file won't work because I had to change the variable name.
